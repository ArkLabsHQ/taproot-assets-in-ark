...
type ArkBoardingTransferOutputDetails struct {
	...
	arkScript      BoardingArkScript
	arkAssetScript BoardingArkAssetScript

	assetTransfer *taprpc.AssetTransfer
}

type ProofOutputDetail struct {
	assetTransfer  *taprpc.AssetTransfer
	transferPacket *tappsbt.VPacket
}

func SpendToBoardingTransaction(assetId []byte, amnt uint64, lockHeight uint32, userTapClient, serverTapClient *TapClient) ArkBoardingTransferOutputDetails {
	...
	return ArkBoardingTransferOutputDetails{btcControlBlock, userScriptKey, userInternalKey, serverScriptKey, serverInternalKey, arkScript, arkAssetScript, sendResp.Transfer}
}
